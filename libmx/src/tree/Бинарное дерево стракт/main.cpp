...
void print(Tree *aTree, int printType)
{
	if(aTree)
	{
		cout << endl << "Количество элементов: " << aTree->count << endl;
		cout << "Корневой элемент: " << aTree->root->value << endl;
...
int main()
{
	...
	do{
		cout << "Введите тип вывода дерева." << endl
		<< "1 - Прямой." << endl << "2 - Симметричный." << endl
		<< "3 - Обратный." << endl << "Введите: ";
		cin >> printType;
	}while(printType < 1 || printType > 3);
	
	cout << endl << "Создание бинарного дерева ..." << endl;
	...
	cout << endl << "Количетсво элементов с одним потомком: " << amount << endl;
	
	cout << endl << "Удаление корневого элемента ..." << endl << endl;
	
	cout << "Предыдущее значение: " << theTree->root->value << endl; 
	deleteElement(theTree, theTree->root->value);
	cout << "Новое значение: " << theTree->root->value << endl;
	
	print(theTree, printType);
	
	cout << endl << "Создание элемента со значением 5151 ..." << endl;
	insertElement(theTree, 5151);
	
	print(theTree, printType);
	
	cout << endl << "Удаление бинарного дерева ..." << endl;
	destroyTree(theTree);
